...
def full_parser(accident_dict, single_year_url, lock):
	...
	for block_year in page_soup_year.find_all('td'):
		for link_year in block_year.find_all('a'):
			...
			for block_accident in page_soup_accident.find_all('tr'):
				...
				lock.release()
			get_accident_info(accident_dict, lock)
...
if (__name__ == '__main__'):
	...
	threads = []
	for task_id in range(len(year_urls)):
		print(year_urls[task_id])
		thread = Thread(target=full_parser, args=(accident_dict, year_urls[task_id], lock))
		thread.start()
