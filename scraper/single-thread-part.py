...
def get_accident_info(accident_dict):			
...
def year_parser(page_soup, n_years):
	n_years_counter = 0
	year_url_array = []
	# Looping all URL to years
	for block in page_soup.find_all('tr'):
		for link in block.find_all('a'):
			if (n_years == n_years_counter):
				return year_url_array
			n_years_counter = n_years_counter + 1
			year_url_array.append(base_url + link.get('href'))


def full_parser(init_time, accident_dict, single_year_url, obj_counter):
	response_year = requests.get(single_year_url)
	year = single_year_url.split("/")[3]
	...
	for block_year in page_soup_year.find_all('td'):
		for link_year in block_year.find_all('a'):
			...
			get_accident_info(accident_dict)
			obj_counter = obj_counter + 1
			print("TOTAL OBJECTS PARSED", obj_counter)
...
if (__name__ == '__main__'):
	...
	year_urls = year_parser(page_soup, n_years)
	print(year_urls)
	for single_year_url in year_urls:
		obj_counter = full_parser(init_time, accident_dict, single_year_url, obj_counter)
		obj_counter_total = obj_counter_total + obj_counter 
